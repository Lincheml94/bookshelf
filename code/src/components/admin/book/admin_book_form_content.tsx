...
const AdminBookFormContent = ({
	...
	const submitForm = async (data: Partial<Book>) => {
		...
		formData.set(
			"published_at",
			normalizeData.published_at as unknown as string,
		);
	...
	return (
		<div className={style["content-dashboard"]}>
            ...
            */}
                ...
                */}
				<form
					className={style.formulaire_crud}
				...
				>
					...
					<p>
						...
						<input
							...
							{...register("title", {
								...
								maxLength: {
									value: 100,
									message: "un titre doit comporter au maximum 100 caractères",
								},
							})}
						/>
					...
					<p className={style.msg_erreur} role="alert">
						{errors.pages?.message ?? serverErrors?.pages}
					</p>
					...
					<p className={style.msg_erreur} role="alert">
						{errors.dimensions?.message ?? serverErrors?.dimensions}
					</p>
					...
					<p className={style.msg_erreur} role="alert">
						{errors.published_at?.message ?? serverErrors?.published_at}
					</p>
					...
					<p className={style.msg_erreur} role="alert">
						{errors.images?.message ?? serverErrors?.images}
					</p>
					...
					<p className={style.msg_erreur} role="alert">
						{errors.isbn?.message ?? serverErrors?.isbn}
					</p>
					...
					<p className={style.msg_erreur} role="alert">
						{errors.print?.message ?? serverErrors?.print}
					</p>
